...
const DeclarationPaletteScreen = () => {
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      // Check the status *before* showing the modal.
      if (palette.statut === 'DC') {
        Alert.alert("Information", "Cette palette a déjà été déclarée.");
        setScannedPalette(null); // Ensure modal doesn't open
      } else {
    ...
    return (
      <View style={{ flex: 1 }}>
        <CameraView
          ...
          onBarcodeScanned={scannedPalette ? undefined : handleBarCodeScanned}
      ...
      >
        <View style={localStyles.modalContainer}>
          <View style={localStyles.modalContent}>
            ...
            {scannedPalette && (
                    <View style={localStyles.modalDetailContainer}>
                        <Text style={localStyles.itemText}><Text style={localStyles.bold}>N° de fabrication:</Text> {scannedPalette.numeroProduction}</Text>
                        <Text style={localStyles.itemText}><Text style={localStyles.bold}>Numéro de palette:</Text> {scannedPalette.numero}</Text>
                <Text style={localStyles.itemText}><Text style={localStyles.bold}>Produit:</Text> {scannedPalette.produitDesignation}</Text>
                        <Text style={localStyles.itemText}><Text style={localStyles.bold}>Type de produit:</Text> {scannedPalette.typeProduitDesignation}</Text>
                        <Text style={localStyles.itemText}><Text style={localStyles.bold}>Article:</Text> {scannedPalette.nomArticle}</Text>
                        <Text style={localStyles.itemText}><Text style={localStyles.bold}>Code article:</Text> {scannedPalette.codeArticle}</Text>
                    </View>
            )}
            {isValidationLoading ? (
